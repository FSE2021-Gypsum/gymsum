...
class Encoder(nn.Module):
    ...
    def forward(self,
                code_ids,
                code_lens):
        layer_outputs, _ = self.transformer(code_ids, code_lens)  # B x seq_len x h
        if self.use_all_enc_layers:
...
class Decoder(nn.Module):
    def __init__(self, args, input_size, include_ext=False):
    ...
    def init_decoder(self,
        ...
        if self.split_decoder:
            state_c = self.transformer_c.init_state(src_lens, max_src_len, ext_len, max_ext_len)
            state_d = self.transformer_d.init_state(src_lens, max_src_len, ext_len, max_ext_len)
            return state_c, state_d
        else:
            return self.transformer.init_state(src_lens, max_src_len, ext_len, max_ext_len)
    ...
    def forward(self,
        ...
        if self.include_ext:
            max_ext_len = ext_out[0].shape[1] if isinstance(ext_out, list) else ext_out.shape[1]

        state = self.init_decoder(memory_len, max_mem_len, ext_len, max_ext_len)
        return self.decode(tgt_pad_mask, tgt_emb, memory_bank, state, ext_outs=ext_out)
