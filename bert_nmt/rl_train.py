...
from bert_nmt.bert_model import Code2NaturalLanguage
from bert_nmt.extract_net import ExtractNet
from bert_nmt.bert_utils import *
from bert_nmt.data_utils import read_examples, convert_examples_to_features
...
def set_defaults(m_config):
    ...
    m_config.rl_model_file = os.path.join(m_config.model_dir, m_config.rl_model_name + '.mdl')
    m_config.model_file = os.path.join(m_config.model_dir, m_config.model_name + '.mdl')
    m_config.log_file = os.path.join(m_config.model_dir, m_config.rl_model_name + suffix + '.txt')
    m_config.pred_file = os.path.join(m_config.model_dir, m_config.rl_model_name + suffix + '.json')

    # 加载pretrain模型
    if m_config.pretrained:
        m_config.pretrained = os.path.join(m_config.model_dir, m_config.pretrained + '.mdl')
        m_config.rl_pretrained = os.path.join(m_config.model_dir, m_config.rl_pretrained + '.mdl')
...
# Train loop
# ------------------------------------------------------------------------------
def train(m_config, data_loader, rl_model, model, global_stats):
    ...
    logger.info('train: Epoch %d | ml_loss = %.2f | '
                'Time for epoch = %.2f (s)' %
                (current_epoch, ml_loss.avg, epoch_time.time()))

    # Checkpoint
    if m_config.checkpoint:
        rl_model.checkpoint(m_config.rl_model_file + '.checkpoint', current_epoch + 1)


# ------------------------------------------------------------------------------
# Validation loops
# ------------------------------------------------------------------------------
def validate_official(m_config, data_loader, rl_model, model, global_stats, mode='dev'):
    ...
    with torch.no_grad():
        ...
        for idx in progress_bar:
            ...
            predictions, targets, copy_info = rl_model.pred(ex, model)
            # print("=" * 49)
            # print(predictions)
            # print(targets)
...
def main(m_config):
    ...
    if include_ast:
        ...
        data_dir = os.path.join(code_seq_config.data.home + m_config.lang)
    ...
    dev_examples = read_examples(m_config, 'dev')
    ...
    if m_config.only_test:
        if m_config.pretrained:
            model = Code2NaturalLanguage.load(m_config.pretrained, code_bert, tokenizer)
        else:
            if not os.path.isfile(m_config.model_file):
                raise IOError('No such file: %s' % m_config.model_file)
            model = Code2NaturalLanguage.load(m_config.model_file, code_bert, tokenizer)
        if m_config.rl_pretrained:
            rl_model = ExtractNet.load(m_config.rl_pretrained, tokenizer)
        else:
            if not os.path.isfile(m_config.model_file):
                raise IOError('No such file: %s' % m_config.model_file)
            rl_model = Code2NaturalLanguage.load(m_config.model_file, code_bert, tokenizer)
    else:
        if m_config.checkpoint and os.path.isfile(m_config.model_file + '.checkpoint'):
            # Just resume training, no modifications.
            logger.info('Found a checkpoint...')
            checkpoint_file = m_config.model_file + '.checkpoint'
            model, start_epoch = Code2NaturalLanguage.load_checkpoint(checkpoint_file, m_config, code_bert, tokenizer,
                                                                      m_config.cuda)
        else:
            ...
            if m_config.pretrained:
                ...
                model = Code2NaturalLanguage(m_config, code_bert, tokenizer)

        if m_config.checkpoint and os.path.isfile(m_config.rl_model_file + '.checkpoint'):
            # Just resume training, no modifications.
            logger.info('Found a extract network checkpoint...')
            checkpoint_file = m_config.rl_model_file + '.checkpoint'
            rl_model, start_epoch = ExtractNet.load_checkpoint(checkpoint_file, m_config, tokenizer, m_config.cuda)
        else:
            # Training starts fresh. But the model state is either pretrained or
            # newly (randomly) initialized.
            if m_config.rl_pretrained:
                logger.info('Using pretrained extract model...')
                rl_model = ExtractNet.load(m_config.pretrained, tokenizer)
            else:
                logger.info('Training extract  model from scratch...')
                rl_model = ExtractNet(m_config, tokenizer)
    ...
    train_loader = None
    if not m_config.only_test:
        train_dataset = data.CommentDataset(train_exs, tokenizer, m_config)
        if include_ast:
            train_dataset.add_vocab(vocab_subtoken=vocab_sub_token, vocab_nodes=vocab_nodes,
    ...
    # TRAIN/VALID LOOP
        ...
        c_rl_model = m_config.extract
        logger.info('-' * 100)
        logger.info('Starting training...')
        stats = {'timer': Timer(), 'epoch': start_epoch, 'best_valid': 0, 'no_improvement': 0}

        if c_rl_model.optimizer in ['sgd', 'adam'] and c_rl_model.warmup_epochs >= start_epoch:
            logger.info("Use warmup lrate for the %d epoch, from 0 up to %s." %
                        (c_rl_model.warmup_epochs, c_rl_model.learning_rate))
            num_batches = m_config.train_len // m_config.batch_size
            warmup_factor = (c_rl_model.learning_rate + 0.) / (num_batches * c_rl_model.warmup_epochs)
            stats['warmup_factor'] = warmup_factor

        for epoch in range(start_epoch, m_config.num_epochs + 1):
            stats['epoch'] = epoch
            if c_rl_model.optimizer in ['sgd', 'adam'] and epoch > c_rl_model.warmup_epochs:
                rl_model.optimizer.param_groups[0]['lr'] = \
                    rl_model.optimizer.param_groups[0]['lr'] * c_rl_model.lr_decay

            train(m_config, train_loader, rl_model, model, stats)
            result = validate_official(m_config, dev_loader, rl_model, model, stats)

            # Save best valid
            if result[m_config.valid_metric] > stats['best_valid']:
                logger.info('Best valid: %s = %.2f (epoch %d, %d updates)' %
                            (m_config.valid_metric, result[m_config.valid_metric],
                             stats['epoch'], rl_model.updates))
                rl_model.save(m_config.rl_model_file)
                stats['best_valid'] = result[m_config.valid_metric]
                stats['no_improvement'] = 0
            else:
                stats['no_improvement'] += 1
                if stats['no_improvement'] >= c_rl_model.early_stop:
                    break
