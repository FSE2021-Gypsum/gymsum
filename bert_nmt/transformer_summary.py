...
class Code2NaturalLanguage(object):
    ...
    def __init__(self, config, src_dict, tgt_dict, state_dict=None):
        # Book-keeping.
        c_model = config.model
        self.c_model = c_model
        self.config = config
        self.src_dict = src_dict
        self.config.src_vocab_size = len(src_dict)
        self.tgt_dict = tgt_dict
        self.config.tgt_vocab_size = len(tgt_dict)
        self.updates = 0
        self.use_cuda = False
        self.parallel = False

        if c_model.model_type == 'rnn':
            self.network = Seq2seq(self.c_model, tgt_dict)
        elif c_model.model_type == 'transformer':
            self.network = Transformer(self.c_model, tgt_dict)
        else:
            raise RuntimeError('Unsupported model: %s' % config.model_type)
    ...
    def init_optimizer(self, state_dict=None, use_gpu=True):
        ...
        """
        if self.c_model.fix_embeddings:
    ...
    def update(self, ex):
        ...
        net_loss = self.network(code_word_rep=code_word_rep,
                                ...
                                max_len=self.c_model.max_tgt_len,
                                blank=blank,
                                fill=fill,
    ...
    def predict(self, ex, replace_unk=False):
        ...
        if replace_unk:
            for i in range(len(predictions)):
                ...
                if self.c_model.model_type == 'transformer':
