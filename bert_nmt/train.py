...
def main(m_config):
    ...
    if m_config.only_test:
        if m_config.pretrained:
            model = Code2NaturalLanguage.load(m_config.pretrained, code_bert, tokenizer, m_config)
        else:
            if not os.path.isfile(m_config.model_file):
                raise IOError('No such file: %s' % m_config.model_file)
            model = Code2NaturalLanguage.load(m_config.model_file, code_bert, tokenizer, m_config)
    else:
    ...
    # TRAIN/VALID LOOP
        ...
        if c_model.optimizer in ['sgd', 'adam'] and c_model.warmup_epochs >= start_epoch:
            ...
            warmup_factor = (c_model.learning_rate + 0.) / (num_batches * c_model.warmup_epochs)
        ...
        for epoch in range(start_epoch, m_config.num_epochs + 1):
            ...
            if c_model.optimizer in ['sgd', 'adam'] and epoch > c_model.warmup_epochs:
                model.optimizer.param_groups[0]['lr'] = \
                    model.optimizer.param_groups[0]['lr'] * c_model.lr_decay

            train(m_config, tokenizer, train_loader, model, stats)
            result = validate_official(m_config, dev_loader, model, tokenizer, stats)
