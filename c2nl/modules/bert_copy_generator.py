...
class CopyGenerator(nn.Module):
    ...
    * :math:`p_{copy}` the probility of copying a particular word.
    ...
    def __init__(self, input_size, pad_index, generator, eps=1e-20):
...
class CopyGeneratorCriterion(object):
    ...
    def __init__(self, vocab_size, force_copy, tokenizer=None, eps=1e-20):
    ...
    def __call__(self, scores, align, target):
        ...
        align_unk = align.eq(unk_token_id).float()
        align_not_unk = align.ne(unk_token_id).float()
        target_unk = target.eq(unk_token_id).float()
        target_not_unk = target.ne(unk_token_id).float()
