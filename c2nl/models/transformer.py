...
class Transformer(nn.Module):
    ...
    def __init__(self, config, tgt_dict):
        """"Constructor of the class."""
        super(Transformer, self).__init__()

        self.name = 'Transformer'
        if len(config.max_relative_pos) != config.nlayers:
            assert len(config.max_relative_pos) == 1
            config.max_relative_pos.expand(config.nlayers)

        self.embedder = Embedder(config)
        self.encoder = Encoder(config, self.embedder.enc_input_size)
        self.decoder = Decoder(config, self.embedder.dec_input_size)
        self.layer_wise_attn = config.layer_wise_attn

        self.generator = nn.Linear(self.decoder.input_size, config.tgt_vocab_size)
